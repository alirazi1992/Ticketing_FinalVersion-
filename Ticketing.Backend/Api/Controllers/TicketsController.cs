...
public class TicketsController : ControllerBase
{
    ...
    public async Task<IActionResult> AddMessage(Guid id, [FromBody] TicketMessageDto request)
    {
        var context = GetUserContext();
        if (context == null)
        {
        ...
        var message = await _ticketService.AddMessageAsync(id, context.Value.userId, request.Message);
        if (message == null)
        {
            return NotFound();
        }
